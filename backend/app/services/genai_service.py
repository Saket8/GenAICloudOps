...
class GenAIService:
    ...
    def __init__(self):
        ...
        # Use currently supported Groq models (updated Dec 2024)
        ...
        # IMPORTANT: Use very short timeout to prevent blocking startup
        try:
            self.redis = redis.Redis(
                ...
                socket_connect_timeout=0.5,  # Reduced from 5s to 0.5s to prevent blocking
                socket_timeout=0.5,
                retry_on_timeout=False
            )
            # Test Redis connection removed for lazy loading
            # self.redis.ping()
            logger.info("Connected to Redis for GenAI caching (lazy)")
        except Exception as e:
    ...
    def client(self):
        ...
        if not self._client and self.groq_api_key:
            try:
                # Import here if optimizing import time, otherwise rely on top-level
                from groq import Groq 
    ...
    async def generate_response(self, request: GenAIRequest) -> GenAIResponse:
        ...
        try:
            ...
            if self.client and self.fallback_model and request.model != self.fallback_model:
    ...
    async def chat_completion(
        ...
        prompt_params = {
            ...
            "user_role": "User",
            "environment_details": "OCI Tenancy",
            "available_resources": "Compute, Networking, Storage, Database",
        }
