...
class AccessAnalyzerService:
    ...
    def kubernetes_service(self):
        ...
        if self._kubernetes_service is None:
            # Create a mock service that returns EMPTY data by default
            # to avoid false critical findings for compartments without Kubernetes
            class MockKubernetesService:
                def __init__(self):
                    self.is_configured = False  # Changed: Not configured by default
                    self.cluster_name = None
                
                def get_rbac_roles(self, namespace=None):
                    # Return empty - no mock data that causes false findings
                    return []
                ...
                def health_check(self):
                    return {"status": "not_configured", "cluster": None}
