...
class OCIVaultService:
    ...
    def __init__(self):
        ...
        # Lazy initialization - do not initialize clients in __init__
        if not OCI_AVAILABLE or getattr(settings, 'USE_DUMMY_VAULT', False):
...
_vault_service_instance = None

def get_vault_service() -> OCIVaultService:
    """Get the OCIVaultService singleton, initializing it if necessary"""
    global _vault_service_instance
    if _vault_service_instance is None:
        logging.getLogger(__name__).info("Initializing OCIVaultService singleton...")
        _vault_service_instance = OCIVaultService()
...
async def get_secret(secret_name: str, force_refresh: bool = False) -> Optional[str]:
    ...
    return await get_vault_service().get_secret(secret_name, force_refresh)
