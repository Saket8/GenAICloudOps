...
export interface AllResourcesResponse {
  ...
  resources: {
    ...
    object_storage_buckets: CloudResource[];
    vaults: CloudResource[];
  };
...
// API Functions (exported for direct use)
export const cloudApi = {
  ...
  // Phase 3: Get resource lifecycle events (stopped duration, state changes)
  getResourceLifecycle: async (resourceId: string, compartmentId: string): Promise<{
    resource_id: string;
    compartment_id: string;
    ...
    const response = await api.get(`/cloud/resources/${resourceId}/lifecycle`, {
...
export function useAllResources(
  ...
  return useQuery({
    ...
    retry: 1, // Don't hammer the server if it fails
